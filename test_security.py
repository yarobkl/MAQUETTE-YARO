#!/usr/bin/env python3
"""
test_security.py — Tests de sécurité complets pour le serveur YARO
...
def setup_server():
    ...
    tmpdir = tempfile.mkdtemp(prefix="yaro_test_")
    ...
    with open(os.path.join(tmpdir, "data.json"), "w") as f:
        json.dump({"hero": {"firstName": "Test", "lastName": "YARO"}}, f)
    ...
    env = os.environ.copy()
    env["PORT"] = str(port)
    env["YARO_PASS"] = "SecurePass#99"
    env["YARO_ORIGINS"] = "http://allowed.example.com"
    env["YARO_TEST"] = "1"  # Active l'endpoint /api/test/reset
...
def test_csrf():
    ...
    status, _, body = req("POST", "/api/save",
        json.dumps({"hero": {"firstName": "Test", "lastName": "YARO"}}).encode(),
...
def main():
    print(f"""
{BLD}{'═'*60}
  🛡️  YARO — Suite de Tests de Sécurité
{'═'*60}{RST}""")
