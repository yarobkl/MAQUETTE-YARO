# Base de données o2switch pour YARO
...
2. Créer une base, par exemple `cpaneluser_yaro`.
3. Créer un utilisateur MySQL, par exemple `cpaneluser_yaro`.
...
MYSQL_PORT=3306
MYSQL_DATABASE=cpaneluser_yaro
MYSQL_USER=cpaneluser_yaro
...
- `yaro_store` pour le contenu, les utilisateurs, contacts et CRM.
- `yaro_photos` pour les images sauvegardées en base.

## 5. Règle importante

`data.json` ne doit plus écraser la base au redémarrage.

La variable `YARO_FORCE_FILE_SYNC=1` force volontairement une restauration depuis `data.json`; elle doit rester à `0` ou vide en production.
