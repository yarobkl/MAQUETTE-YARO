...
def replacements_from_config(config: dict) -> dict[str, str]:
    ...
    old_full_name = brand.get("old_full_name", "YARO Maquette")
    new_full_name = brand.get("new_full_name", "Nom Complet du Client")

    return {
        old_full_name: new_full_name,
        "maquette-yaro.fr": config.get("public_domain", "https://client.cg").replace("https://", ""),
    }
...
YARO_USER={config.get('admin_user', 'admin')}
YARO_PASS={config.get('admin_password_placeholder', 'CHANGE_ME')}
ADMIN_SECRET_PATH={config.get('admin_secret_path', 'espace-prive-client')}
YARO_ORIGINS={origins}
