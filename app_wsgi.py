"""WSGI application used by the o2switch Passenger wrapper.
...
def _bootstrap() -> None:
    ...
    for name in ("init_users", "load_blocked_ips", "load_attack_scores"):
        fn = getattr(yaro_server, name, None)
        if callable(fn):
            fn()

    if os.environ.get("YARO_PASSENGER_BOOT_SERVICES") != "1":
        return
    ...
    try:
        ...
        mon = getattr(yaro_server, "_MON", None)
        if mon and hasattr(mon, "init_db") and hasattr(mon, "start_scheduler"):
            mon.init_db()
            mon.start_scheduler(
                get_sessions_fn=lambda: len(getattr(yaro_server, "ACTIVE_SESSIONS", [])),
                get_blocked_fn=lambda: len(getattr(yaro_server, "BLOCKED_IPS", [])),
            )
    except Exception as exc:
        print(f"[PASSENGER] Services arrière-plan ignorés: {exc}", flush=True)


class _PassengerHandler(yaro_server.YaroHandler):
