...
def _send_notif_email(entry: dict, notif_type: str):
    ...
    labels = {
        "yaro_audiences":      "📋 Demande d'audience",
        "yaro_contacts":       "✉️ Message de contact",
        "yaro_newsletter":     "📩 Inscription newsletter",
        "yaro_commande_livre": "📚 Commande de livre",
    }
...
YARO_TEST    = os.environ.get("YARO_TEST", "") == "1"  # Mode test uniquement
ADMIN_USER      = os.environ.get("YARO_USER", "admin")
ADMIN_PASS      = os.environ.get("YARO_PASS", "")
PROTECTED_USER  = os.environ.get("YARO_PROTECTED", "rodrin")
...
def _totp_uri(secret: str, username: str, issuer: str = "YARO") -> str:
...
# ══════════════════════════════════════════════════════════
# ██  MODULE ANTI-INTRUSION — YARO SECURITY ENGINE     ██
# ══════════════════════════════════════════════════════════
...
def record_attack(ip: str, event_type: str, score: int, detail: str = ""):
    ...
    if entry["score"] >= ATTACK_BAN_THRESHOLD and ip not in BLOCKED_IPS and not _is_trusted_ip(ip):
        ...
        print(f"[YARO] 🚫 IP bannie automatiquement : {ip} (score={entry['score']})")
...
def save_users(users):
    ...
    try:
        ...
        print(f"[YARO] Erreur sauvegarde users fichier : {e}")
...
def init_users():
    ...
    if not existing:
        ...
        if not password:
            ...
            print(f"[YARO] ⚠️  YARO_PASS non défini — mot de passe généré : {password}")
            print(f"[YARO] ⚠️  Définissez YARO_PASS=<votre-mot-de-passe> pour un mot de passe fixe.")
        save_users([{
            "username": ADMIN_USER,
        ...
        print(f"[YARO] 📁 Compte admin créé ({ADMIN_USER})")
    elif not ADMIN_PASS:
        ...
        if on_railway:
            print("[YARO] ❌ YARO_PASS est obligatoire en production. Ajoutez la variable d'environnement.")
            import sys; sys.exit(1)
        else:
            print(f"[YARO] ⚠️  YARO_PASS non défini — définissez la variable d'environnement.")
...
def save_news(data: dict):
    try:
        ...
        print(f"[YARO] Erreur sauvegarde news: {e}")
...
# Table unique yaro_store : key TEXT PRIMARY KEY, data TEXT (JSON), updated_at
        ...
        if is_closed or conn_backend != backend:
            if backend == "postgresql":
                ...
                with conn.cursor() as cur:
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS yaro_store (
                    ...
                    """)
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS yaro_photos (
                            id           TEXT PRIMARY KEY,
                            data         BYTEA NOT NULL,
...
def _pg_load(key: str):
    ...
    try:
        ...
        with conn.cursor() as cur:
            if backend == "mysql":
                cur.execute("SELECT data FROM yaro_store WHERE `key` = %s", (key,))
            else:
                cur.execute("SELECT data FROM yaro_store WHERE key = %s", (key,))
            row = cur.fetchone()
...
def _pg_save(key: str, value) -> bool:
    ...
    try:
        ...
        with conn.cursor() as cur:
            if backend == "mysql":
                cur.execute("""
                    INSERT INTO yaro_store (`key`, data, updated_at)
                    VALUES (%s, %s, NOW())
...
def _pg_save_photo(photo_id: str, data_bytes: bytes, content_type: str = "image/jpeg") -> bool:
    ...
    try:
        ...
        with conn.cursor() as cur:
            if backend == "mysql":
                cur.execute("""
                    INSERT INTO yaro_photos (id, data, content_type)
                    VALUES (%s, %s, %s)
...
def _pg_load_photo(photo_id: str):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT data, content_type FROM yaro_photos WHERE id = %s", (photo_id,))
            row = cur.fetchone()
            if row:
...
def list_backups(limit: int = 20):
    """Retourne les sauvegardes locales créées par backup_yaro.py."""
...
def run_backup_now():
    ...
    import backup_yaro
    dest = backup_yaro.export_backup()
    backup_yaro.prune_old_backups(14)
...
def save_contacts(contacts: list):
    ...
    try:
        ...
        print(f"[YARO] Erreur sauvegarde contacts : {e}")
    if not db_ok and not file_ok:
...
def save_crm(data: dict):
    ...
    try:
        ...
        print(f"[YARO] Erreur sauvegarde CRM: {e}")
    if not db_ok and not file_ok:
...
def _migrate_files_to_db():
    ...
    force_file_sync = os.environ.get("YARO_FORCE_FILE_SYNC", "") == "1"
...
# ── Handler ────────────────────────────────────────────────
class YaroHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/contacts":
            ...
            def _is_audience(c):
                return c.get("type") == "yaro_audiences" or c.get("source") == "yaro_audiences"
            def _is_newsletter(c):
                return c.get("type") == "yaro_newsletter" or c.get("source") == "yaro_newsletter"
    ...
    def do_POST(self):
        ...
        if YARO_TEST and path == "/api/test/reset":
            ...
            print(f"[SECURITY] /api/test/reset appelé par {ip} — YARO_TEST=1")
        ...
        if path == "/api/login":
            ...
            try:
                ...
                if user and _verify_password(password, user.get("password_hash", "")):
                    ...
                    if not user["password_hash"].startswith("pbkdf2:"):
                        ...
                        for u in users:
                            if u["username"] == username:
                                ...
                                break
                        save_users(users)
                        print(f"[YARO] 🔒 Hash mis à jour (pbkdf2) pour : {username}")
                    ...
                    print(f"[YARO] 🔓 Connexion : {username} ({user['role']}) — {datetime.now().strftime('%H:%M:%S')} {'[IP de confiance]' if is_trusted else ''}")
                    ...
                    print(f"[YARO] ⛔ Tentative échouée : {username} — {datetime.now().strftime('%H:%M:%S')}")
        ...
        if path == "/api/chat":
            ...
            try:
                ...
                nom  = f"{hero.get('firstName','')} {hero.get('lastName','')}".strip() or "YARO Maquette"
                ...
                # ── Parcours / formation / carrière ───────────────────────────
                    ...
                    if len(paras) > 1:
                        ...
                        reply = f"{nom} est Docteur en droit et Inspecteur principal du Trésor public. Il a exercé plusieurs fonctions ministérielles importantes au Congo avant d'être élu Site de démonstration."
                ...
                # ── Fonctions / titre / ministre ──────────────────────────────
                    ...
                    reply = f"{nom} est actuellement {role}. Il est Projet institutionnel de démonstration depuis 2017, et a exercé les fonctions de Ministre des Finances avant de prendre en charge la Justice."
                ...
                # ── Ewo / circonscription / Cuvette ──────────────────────────
                    ...
                    reply = f"{nom} est le Projet institutionnel de démonstration, dans la Cuvette-Ouest (République du Congo). Il s'engage personnellement auprès des populations locales, défend leurs intérêts à l'Assemblée Nationale et œuvre pour le développement de cette région."
                ...
                # ── Assemblée nationale / parlement ───────────────────────────
                    ...
                    reply = f"{nom} siège à l'Assemblée Nationale en tant que Projet institutionnel de démonstration depuis 2017. Il y défend activement les intérêts de sa circonscription et porte des textes de loi importants, notamment dans le domaine judiciaire."
                ...
                # ── Langues étrangères / lingala / kituba / anglais ──────────
                    ...
                    reply = "I'm DA, the virtual assistant of Minister YARO's official website. I mainly respond in French. Please write your question in French and I'll be happy to help you. — DA"
                ...
                # ── Compliments / félicitations ───────────────────────────────
                    ...
                    reply = random.choice([
                        ...
                        "C'est très gentil ! Je suis là pour vous aider. Posez-moi toutes vos questions sur YARO Maquette. — DA",
                ...
                # ── Quiz ──────────────────────────────────────────────────────
                elif any(w in q for w in ["quiz", "jeu", "devinette", "question sur yaro", "tester", "test sur", "je sais tout"]):
                    reply = random.choice([
    ...
    "url": "https://yaro-ref.cg/{secrets.token_hex(6)}"
        ...
        if path == "/api/contact":
            try:
                ...
                _notif_type = {"yaro_audiences": "audience", "yaro_contacts": "contact"}.get(etype, "contact")
                if etype == "yaro_audiences" and entry.get("objet") == "Réclamation":
                ...
                if email_contact or phone_contact:
                    try:
                        source_map = {
                            "yaro_newsletter":      "newsletter",
                            "yaro_audiences":       "audience",
                            "yaro_contacts":        "contact",
                            "yaro_commande_livre":  "livre",
                        }
                        action_map = {
                        ...
                        is_newsletter = etype == "yaro_newsletter"
                        now_str      = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        with _CRM_LOCK:
                            crm = load_crm()
                            existing = None
        ...
        if path == "/api/upload":
            ...
            print(f"[YARO] 📷 Image uploadée : {safe_name}")
        ...
        if path == "/api/editorial/generate":
            ...
            try:
                ...
                editorial = None
                if key:
                    prompt = f"""Tu es un assistant éditorial intégré au système de veille YARO IA du site du Député YARO Maquette (Congo-Brazzaville).
...
{{
  ...
  "sources": ["{source_nm} — {url_src}"]
        ...
        # ── /api/youtube/* — module retiré de l'espace admin ──
        if path in ("/api/youtube/generate", "/api/youtube/save", "/api/youtube/delete"):
            self._json({"ok": False, "message": "Module YouTube IA désactivé"}, 410)
            return
        ...
        if path == "/api/crm/import":
            ...
            try:
                ...
                for raw in all_contacts:
                    ...
                    is_nl    = raw_type == "yaro_newsletter"
                    contact  = {
    ...
    def version_string(self):
        return "YARO/1.0"  # Masquer la version Python
...
def generate_self_signed_cert():
    ...
    try:
        ...
        result = subprocess.run([
            ...
            "-days", "365", "-nodes",
            "-subj", "/C=CG/ST=Cuvette/L=Ewo/O=Yaro/CN=localhost",
            "-addext", "subjectAltName=IP:127.0.0.1,DNS:localhost"
        ], capture_output=True, timeout=30)
        if result.returncode == 0:
            print("[YARO] ✅ Certificat SSL auto-généré (cert.pem / key.pem)")
            return True
        else:
            print("[YARO] ⚠️  Impossible de générer le certificat SSL (openssl requis)")
            return False
    except Exception:
        print("[YARO] ⚠️  openssl non disponible — SSL désactivé")
        return False
...
def resolve_ssl_certs():
    ...
    env_cert = os.environ.get("YARO_CERT", "")
    env_key  = os.environ.get("YARO_KEY", "")
...
def start_monitor():
    ...
    if not os.path.isfile(monitor_path):
        print("[YARO] ⚠️  monitor.py introuvable — veille désactivée")
        return
    ...
    if os.path.isfile(pid_file):
        try:
            pid = int(open(pid_file).read().strip())
            os.kill(pid, 0)
            print(f"[YARO] 🤖 YARO IA déjà actif (PID {pid})")
            return
    ...
    print(f"[YARO] 🤖 YARO IA lancé (PID {proc.pid}) — logs : {log_path}")
            ...
            try:
                ...
                if os.path.isfile(pid_file):
                    try:
                        ...
                        pass
                if not alive:
                    print("[YARO] ⚠️  YARO IA inactif — redémarrage automatique…", flush=True)
                    start_monitor()
            except Exception as e:
                print(f"[YARO] Watchdog erreur : {e}", flush=True)
...
if __name__ == "__main__":
    ...
    if _MON:
        ...
        print("[YARO] 📊 Monitoring démarré (monitoring.db)")
    ...
    _force_http = os.environ.get("YARO_FORCE_HTTP", "") == "1"
...
╔══════════════════════════════════════════════╗
  ...
  ╚══════════════════════════════════════════════╝
    ...
    server = _ThreadedHTTPServer(("0.0.0.0", PORT), YaroHandler)
