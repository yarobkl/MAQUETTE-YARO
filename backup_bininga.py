#!/usr/bin/env python3
"""Sauvegarde durable du contenu YARO.

Exporte la base MySQL/PostgreSQL dans backups/ avec :
- yaro_store.json : contenus, utilisateurs, contacts, CRM
...
def export_backup() -> Path:
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(f"SELECT {key_col}, data FROM yaro_store ORDER BY {key_col}")
        rows = cur.fetchall()
        store = {}
        ...
        (dest / "yaro_store.json").write_text(
            json.dumps(store, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )

        cur.execute("SELECT id, data, content_type FROM yaro_photos ORDER BY id")
...
def prune_old_backups(keep: int) -> None:
    if keep <= 0 or not BACKUP_ROOT.exists():
        return
    backups = sorted(p for p in BACKUP_ROOT.glob("yaro-*") if p.is_dir())
    for old in backups[:-keep]:
        shutil.rmtree(old, ignore_errors=True)


def main() -> int:
    parser = argparse.ArgumentParser(description="Sauvegarde DB/photos YARO")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERREUR sauvegarde YARO: {exc}", file=sys.stderr)
        raise SystemExit(1)
