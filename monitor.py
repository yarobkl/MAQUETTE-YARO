#!/usr/bin/env python3
"""
YARO IA — Agent de surveillance des actualités concernant YARO
====================================================================
...
le Député/Ministre YARO Maquette.
...
# Requêtes de recherche — Actualités Yaro
QUERIES = [
    '"Ange YARO"',
    '"YARO" Congo',
    '"Yaro" "Garde des Sceaux" Congo',
    '"Yaro" député "Brazzaville"',
    '"Yaro" ministre justice Congo 2026',
    '"YARO" Congo Brazzaville',
    # Presse congolaise ciblée (fallback si RSS indisponible)
    '"Yaro" site:lesdepechesdebrazzaville.fr',
    '"Yaro" site:adiac-congo.com',
    '"Yaro" site:congo-site.com OR site:acp.cg OR site:lesechosducongo.net',
]
...
NITTER_QUERIES = [
    "Yaro Congo",
    "Yaro ministre justice",
    "YARO",
]

# Requêtes Google News ciblées réseaux sociaux
SOCIAL_GOOGLE_QUERIES = [
    '"Yaro" twitter OR facebook OR instagram Congo',
    '"YARO" discours OR interview OR déclaration',
    '"Yaro" ministre congo vidéo OR communiqué 2026',
]

# YouTube — recherche via Google News (YouTube n'a pas de RSS de recherche public)
YOUTUBE_GOOGLE_QUERIES = [
    '"Yaro" Congo site:youtube.com',
    '"ministre Yaro" site:youtube.com OR site:youtu.be',
]

HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (compatible; YaroVeille/1.0; "
        "+https://maquette-yaro.fr) Python/3"
    ),
...
def fetch_extra_rss(rss_url: str) -> list[dict]:
        ...
        if proxy_data:
            try:
                ...
                if jd.get("status") == "ok":
                    ...
                    keywords = ["yaro", "ange aimé", "garde des sceaux"]
    ...
    # Filtrer seulement ceux qui concernent Yaro
    keywords = ["yaro", "ange aimé", "garde des sceaux"]
    return [a for a in articles if any(k in (a["title"] + a["summary"]).lower() for k in keywords)]
        ...
        payload = json.dumps({
            ...
            "messages": [{
                ...
                "content": (
                    ...
                    f"en te concentrant sur ce qui concerne YARO Maquette.\n\n"
...
def send_email_notification(new_articles: list[dict]):
    ...
    subject = f"[Veille YARO] {count} nouvelle(s) info(s) détectée(s)"
    ...
    body_parts.append(f'<p><a href="https://maquette-yaro.fr/{_admin_path}">→ Ouvrir l\'espace admin</a></p>')
    html_body = "\n".join(body_parts)
...
def run_cycle(data: dict, custom_query: str = "") -> list[dict]:
    ...
    # Google News — Actualités Yaro
    for query in QUERIES:
        _log(f"[Yaro] {query}")
        _add(fetch_google_news(query), "yaro")
        time.sleep(2)
    ...
    for rss_url in EXTRA_RSS:
        ...
        _add(fetch_extra_rss(rss_url), "yaro")
        time.sleep(1)
...
def main():
    _log("=== YARO IA — Agent YARO démarré ===")
