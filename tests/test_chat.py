...
def _reset():
    """Remet à zéro les compteurs de sécurité entre les tests (mode YARO_TEST=1)."""
    try:
...
def test_chat_bonjour():
    ...
    assert any(w in reply for w in ["da", "yaro", "bonjour", "bienvenue"]), \
...
def test_chat_presentation():
    """Le chatbot peut présenter le Ministre YARO."""
    status, data = chat("Qui est Yaro ?")
    assert status == 200
    reply = data["reply"].lower()
    assert "yaro" in reply, f"Réponse doit mentionner YARO : {data['reply']}"
...
def test_chat_avec_historique():
    ...
    status, data = chat("Qui est Yaro ?", history=history)
    assert status == 200
    assert "reply" in data
