"""
Tests automatiques — Serveur YARO
...
def test_login_correct():
    ...
    user = os.environ.get("YARO_USER", "admin")
    pwd  = os.environ.get("YARO_PASS", "yaro2025")
...
def test_news_mark_read():
    ...
    test_item = {"id": "test_read_001", "title": "Test", "url": "https://example.com", "source": "test",
                 "published": "", "found_at": "", "summary": "", "read": False, "category": "yaro"}
...
def test_news_mark_all_read():
    ...
    items = [
        {"id": f"test_all_{i}", "title": f"Art {i}", "url": "https://example.com",
         "source": "test", "published": "", "found_at": "", "summary": "", "read": False, "category": "yaro"}
        for i in range(3)
    ]
    with open(news_file, "w") as f:
...
def test_monitor_category_field():
    ...
    assert "yaro" in source, "La catégorie 'yaro' doit exister"
...
def test_admin_html_veille_run():
    ...
    assert "filter-yaro" in admin_html, "Le filtre Yaro doit exister dans admin.html"
