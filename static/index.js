...
function fSub(e, f) {
  ...
  sendForm("yaro_audiences", FORMSPREE.audience, fd,
...
function toggleGeoFields(sel) {
  ...
  const titles = {
    "Demande d'audience":  ["Demande de rendez-vous", "Expliquez l'objet de votre demande — notre équipe vous contactera sous 48h"],
    "R\u00e9clamation":   ["Signalement officiel au Client", "Géolocalisez le problème et ajoutez une photo — le Client et toute son équipe seront alertés immédiatement"],
    "Question commerciale":  ["Poser une question au Client", "Rédigez votre question, le Client ou son équipe vous répondra directement"],
  ...
  const placeholders = {
    "Demande d'audience":  "Expliquez pourquoi vous souhaitez rencontrer le Client, votre situation et l'objet précis de votre demande…",
    "Question commerciale":  "Rédigez votre question pour le Site de démonstration…",
    ...
    async pos => {
      ...
      try {
        const r = await fetch(
          ...
          { headers: { "User-Agent": "YaroDepute/1.0" } }
        );
...
function fCt(e, f) {
  e.preventDefault();
  sendForm("yaro_contacts", FORMSPREE.contact, new FormData(f),
...
async function submitOrder(e, f) {
  ...
  try { const l=JSON.parse(localStorage.getItem("yaro_commandes")||"[]"); l.push(entry); localStorage.setItem("yaro_commandes",JSON.stringify(l)); } catch(_){}
...
async function subNewsletter(e, f) {
  ...
  btn.disabled = true; btn.textContent = "⏳ Envoi…";
  const entry = { email, type:"yaro_newsletter", _date: new Date().toLocaleString("fr-FR"), _id: Date.now()+"-"+Math.random().toString(36).slice(2,8) };
  try { await fetch("/api/contact",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(entry)}); } catch(_){}
  try { const l=JSON.parse(localStorage.getItem("yaro_newsletter")||"[]"); l.push(entry); localStorage.setItem("yaro_newsletter",JSON.stringify(l)); } catch(_){}
  f.style.display = "none";
