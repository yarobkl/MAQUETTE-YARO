...
const SESSION_STORAGE_KEY = "yaro_session";
...
async function syncMessages() {
  try {
    ...
    const mapping = [
      ["yaro_audiences", data.audiences || []],
      ["yaro_contacts",  data.contacts  || []]
    ];
...
function _refreshDashboardLocal() {
  const aud  = getAll("yaro_audiences");
  const ct   = getAll("yaro_contacts");
...
function renderAudiences() {
  const all  = getAll("yaro_audiences").filter(m => m.objet !== "Réclamation");
  const list = audFilter === "all" ? all : all.filter(m => (m._status||"en_attente") === audFilter);
  renderMsgList("list-audiences", list, "yaro_audiences", "status3");
}
...
function renderReclamations() {
  ...
  renderMsgList("list-reclamations", list, "yaro_audiences", "status3");
}
...
function renderContacts() {
  const all  = getAll("yaro_contacts");
  const list = ctFilter === "all" ? all : all.filter(m => (m._status||"non_lu") === ctFilter);
  renderMsgList("list-contacts", list, "yaro_contacts", "status2");
}
...
function renderMsgList(containerId, list, storageKey, mode) {
  ...
  el.innerHTML = list.map(m => {
    ...
    const replyBtn = m.email
      ? `<a class="sbtn sbtn-progress" href="mailto:${m.email}?subject=${encodeURIComponent('Réponse — Cabinet YARO')}&body=${encodeURIComponent('Bonjour ' + (m.prenom||'') + ' ' + (m.nom||'') + ',\n\n')}" style="text-decoration:none">Répondre</a>`
...
function setStatus(storageKey, idOrIdx, status) {
  ...
  refreshDashboard();
  if (storageKey === "yaro_audiences") { renderAudiences(); renderReclamations(); }
  else renderContacts();
...
function setNewsFilter(f) {
  _newsFilter = f;
  // Onglet Yaro
  const tabB = document.getElementById("tab-yaro");
...
const CAT_LABELS = {
  yaro:     { label: "Yaro",      color: "rgba(200,16,46,.8)",    bg: "rgba(200,16,46,.12)" },
...
function renderNewsItems() {
  ...
  const filtered = _newsFilter === "all"
    ? _newsItems
    : _newsItems.filter(a => (a.category || "yaro") === _newsFilter);

  // Mise à jour compteurs onglets
  const bCount = _newsItems.filter(a => (a.category || "yaro") === "yaro").length;
  const aCount = _newsItems.length;
  const bUnread = _newsItems.filter(a => (a.category || "yaro") === "yaro" && !a.read).length;
  const aUnread = _newsItems.filter(a => !a.read).length;
  const tbc = document.getElementById("tab-yaro-count");
  ...
  list.innerHTML = sorted.map(a => {
    const isRead   = a.read;
    const cat      = CAT_LABELS[a.category] || CAT_LABELS.yaro;
...
const PANEL_TITLES = {
  ...
  veille:"YARO IA — Actualités Yaro",
...
async function resetSystem(targets) {
  ...
  try {
    ...
    if (targets.includes("contacts")) {
      localStorage.removeItem("yaro_audiences");
      localStorage.removeItem("yaro_contacts");
    }
    syncMessages().then(() => refreshDashboard());
...
function exportCSV(storageKey, type) {
  ...
  a.href     = url;
  a.download = `yaro_${type}_${new Date().toISOString().slice(0,10)}.csv`;
  a.click();
...
function _crmSaveBackup(contacts) {
  try {
    localStorage.setItem("yaro_crm_backup", JSON.stringify({
...
async function _crmRestoreFromBackup() {
  const raw = localStorage.getItem("yaro_crm_backup");
  if (!raw) return 0;
  let contacts;
...
function crmDetail(id) {
  ...
  body.innerHTML = `
    ...
    <div style="display:flex;gap:8px;margin-top:20px;padding-top:16px;border-top:1px solid rgba(255,255,255,.06)">
      ${c.email ? `<a href="mailto:${esc(c.email)}?subject=${encodeURIComponent('Réponse Cabinet YARO')}" class="sbtn sbtn-progress" style="text-decoration:none">Envoyer un email</a>` : ""}
...
async function loadEdNewsPicker() {
  ...
  try {
    ...
    const data = await res.json();
    const items = (data.items || []).filter(a => a.category === "yaro" || (a.title + (a.summary||"")).toLowerCase().includes("yaro")).slice(0, 20);
    _edNewsPicker = items;
    if (!items.length) { box.innerHTML = '<div style="text-align:center;color:rgba(255,255,255,.3);padding:16px 0;font-size:12px">Aucun article Yaro disponible dans la veille.<br><small>Lancez une recherche dans YARO IA.</small></div>'; return; }
...
function _addNotif(type, data) {
  ...
  if (Notification.permission === "granted" && document.visibilityState !== "visible") {
    try {
      const n = new Notification("YARO Admin — " + meta.title, {
        body: desc || "",
        icon: "/images/yaro.jpg.jpg.jpg",
        tag: type,
      });
