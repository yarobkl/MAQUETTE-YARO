...
YARO_USER=admin
YARO_PASS=mot_de_passe_admin_tres_fort
ADMIN_SECRET_PATH=espace-prive-client-long-secret
YARO_ORIGINS=https://client.cg,https://www.client.cg

NOTIF_EMAIL_FROM=
NOTIF_EMAIL_PASS=
NOTIF_EMAIL_TO=
YARO_FORCE_FILE_SYNC=0
```

Note : les noms `YARO_*` existent encore dans le code. Pour aller plus
...
- Renommer les constantes `YARO_*` en constantes generiques.
- Sortir les labels "YARO" restants dans un fichier `client.json`.
